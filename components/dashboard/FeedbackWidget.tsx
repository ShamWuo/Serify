    ...
    if (!isOpen) {
        return (
            <button
                ...
                className="fixed bottom-6 left-6 z-50 w-12 h-12 bg-[var(--accent)] text-white rounded-full shadow-lg flex items-center justify-center hover:scale-110 active:scale-95 transition-transform group"
                title="Send Beta Feedback"
            >
                <MessageSquarePlus size={24} />
                <span className="absolute left-full ml-3 px-3 py-1.5 bg-[var(--surface)] text-[var(--text)] text-xs font-bold rounded-lg border border-[var(--border)] shadow-sm opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none -translate-x-2 group-hover:translate-x-0">
                    Send Feedback
                </span>
            </button>
        );

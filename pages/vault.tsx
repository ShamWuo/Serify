...
function MasteryDot({ state, size = 10 }: { state: MasteryState; size?: number }) {
    ...
    return (
        <span
            className={`inline-block rounded-full shrink-0 ${cfg.dot}`}
    ...
    if (!node)
        return (
            <div className="flex flex-col h-full items-center justify-center p-8 text-center">
                ...
                <button onClick={onClose} className="mt-4 text-[var(--accent)] text-sm">
                    Close
                </button>
            </div>
        );
                ...
                <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-[var(--muted)] mt-1">
                            ...
                            {node.session_count} {node.session_count === 1 ? 'session' : 'sessions'}{' '}
            ...
            <div className="flex-1 px-6 py-5 space-y-7">
                ...
                {history.length > 0 && (
                    <div>
                        ...
                        <div className="flex items-center gap-0 overflow-x-auto pb-2">
                            {history.map((entry, i) => {
                                const cfg =
                                    MASTERY_CONFIG[entry.state as MasteryState] ||
                                    MASTERY_CONFIG.revisit;
                                ...
                                return (
                                    <div key={i} className="flex items-center shrink-0">
                                        {i > 0 && (
                                            <div
                                                className={`w-8 h-0.5 ${improved ? 'bg-[#2A5C45]' : 'bg-[var(--border)]'}`}
                                            />
                                        )}
                    ...
                    <div>
                        ...
                        {(node.synthesis as any).persistentGap && (
                            <div className="mt-3 p-3 bg-[#B8860B]/10 border border-[#B8860B]/20 rounded-xl">
                                <p className="text-xs font-bold text-[#B8860B] mb-1">
                                    Persistent Gap
                                </p>
                        ...
                        {(node.synthesis as any).improvement && (
                            <div className="mt-3 p-3 bg-[#2A5C45]/10 border border-[#2A5C45]/20 rounded-xl">
                                <p className="text-xs font-bold text-[#2A5C45] mb-1">
                                    Recent Improvement
                                </p>
                                <p className="text-xs text-[var(--text)] leading-relaxed">
                                    {(node.synthesis as any).improvement}
                            ...
                            {sessions.map((s) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <ArrowRight
                                            size={14}
                                            className="text-[var(--muted)] group-hover:text-[var(--accent)] shrink-0 ml-2 transition-colors"
                                        />
                                    </Link>
                                );
                            })}
                ...
                <div>
                    ...
                    <p className="text-[11px] text-[var(--muted)] mb-3">
                        Launch a learning mode focused specifically on this concept.
                    </p>
                    ...
                    >
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                <span className="text-[13px] font-bold block text-amber-200 group-hover:text-amber-300 transition-colors">
                                    ✦ Learn This Concept
                                </span>
                                <span className="text-[10px] opacity-80">
                                    Build a curriculum around this
                                </span>
                            </div>
                        </div>
                        <ArrowRight
                        ...
                        {[
                            {
                                ...
                                href: sessions[0] ? `/learn/${sessions[0].id}/flashcards` : '#',
                                cost: 1
                            },
                            {
                                label: 'Explain It',
                                icon: <MessageSquare size={16} />,
                                href: sessions[0] ? `/learn/${sessions[0].id}/explain` : '#',
                                cost: 1
                            },
                            {
                                label: 'Feynman',
                                icon: <BrainCircuit size={16} />,
                                href: sessions[0] ? `/learn/${sessions[0].id}/feynman` : '#',
                                cost: 2
                            },
                            {
                                label: 'AI Tutor',
                                icon: <Bot size={16} />,
                                href: sessions[0] ? `/learn/${sessions[0].id}/tutor` : '#',
                                cost: 1
                            }
                        ].map((mode) => (
                            <Link
                            ...
                            {node.hint_request_count > 0 && (
                                <p>
                                    ...
                                    <span className="text-[var(--text)] font-medium">
                                        {node.hint_request_count}{' '}
                                        {node.hint_request_count === 1 ? 'time' : 'times'}
                                    </span>
                                </p>
                            )}
                            {node.skip_count > 0 && (
                                <p>
                                    Skipped:{' '}
                                    <span className="text-[var(--text)] font-medium">
                                        {node.skip_count} {node.skip_count === 1 ? 'time' : 'times'}
                                    </span>
                                </p>
                            )}
                            {node.hint_request_count >= 2 && (
                                <p className="text-xs text-[#B8860B] mt-2 italic">
                                    You&apos;ve needed hints across multiple sessions — this concept
                                    may benefit from a dedicated study session.
                                </p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="max-w-[860px] mx-auto px-6 py-8">
                                ...
                                {(
                                    ...
                                    >
                                        ...
                                        <span className="text-sm text-[var(--text)] font-medium">
                                            {stats[state] || 0} {MASTERY_CONFIG[state].label}
                                        </span>
                                    </button>
                                ))}
                            </div>
                        ...
                        {loading && (
                            <div className="space-y-3">
                                {[...Array(4)].map((_, i) => (
                                    <div
                                        key={i}
                                        className="h-14 bg-[var(--surface)] border border-[var(--border)] rounded-2xl animate-pulse"
                                    />
                                ))}
                            </div>
                        )}
                        ...
                        {!loading && !hasAnyConcepts && (
                            <div className="flex flex-col items-center justify-center py-24 text-center">
                                ...
                                {backfilling ? (
                                    <>
                                        <h2 className="text-2xl font-display text-[var(--text)] mb-3">
                                            Importing your concepts…
                                        </h2>
                                    ...
                                    <>
                                        <h2 className="text-2xl font-display text-[var(--text)] mb-3">
                                            Your Concept Vault is empty
                                        </h2>
                                        <p className="text-[var(--muted)] text-[15px] max-w-sm leading-relaxed mb-8">
                                            Complete your first session and Serify will
                                            automatically track every concept you encounter.
                                        </p>
                                        <div className="flex gap-3">
                                            ...
                                            >
                                                {backfillDone
                                                    ? 'Already imported'
                                                    : 'Import from past sessions'}
                                            </button>
                                            <Link
                                                href="/analyze"
                                                className="px-6 py-2.5 bg-[var(--accent)] text-white rounded-xl font-medium hover:bg-[var(--accent)]/90 transition-all shadow-md shadow-[var(--accent)]/20"
                                            >
                                                Analyze Something →
                                            </Link>
                                        </div>
                                ...
                                {tab === 'needs_work' ? (
                                    <>
                                        ...
                                        <p className="text-[var(--muted)] text-sm">
                                            All your concepts are Solid or Developing. Keep it up.
                                        </p>
                                    </>
                                ) : (
                                    <p className="text-[var(--muted)]">
                                        Complete more sessions to build your Solid concept
                                        collection.
                                    </p>
                                )}
                            </div>
                            ...
                            (viewMode === 'flat' ? (
                                ...
                                <div className="space-y-4">
                                    {topicGroups.map(({ topic, nodes: topicNodes }) => {
                                        const needsWork = topicNodes.filter(
                                            (n) =>
                                                n.current_mastery === 'shaky' ||
                                                n.current_mastery === 'revisit'
                                        ).length;
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center gap-3 shrink-0">
                                                        {needsWork > 0 && (
                                                            <span className="text-xs font-bold text-[#B8860B] bg-[#B8860B]/10 px-2 py-0.5 rounded-full">
                                                                {needsWork} need work
                                                            </span>
                                                        )}
                                                        <span className="text-xs text-[var(--muted)]">
                                                            {topicNodes.length} concept
                                        ...
                                        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl overflow-hidden">
                                            <div className="px-5 py-4 border-b border-[var(--border)] flex items-center gap-2">
                                                <ChevronDown
                                                    ...
                                                    className="text-[var(--muted)]"
                                                />
                                                <span className="font-bold text-[var(--muted)] text-sm">
                                                    Uncategorized
                                                </span>
                                                <span className="text-xs text-[var(--muted)] ml-auto">
                                                    {uncategorized.length} concept
...
function ConceptRow({
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center gap-3 shrink-0">
                <span className={`text-xs font-bold ${cfg.color} hidden sm:block`}>
                    {cfg.label}
                </span>
                <span className="text-xs text-[var(--muted)]">
                    {node.session_count || 0} {node.session_count === 1 ? 'session' : 'sessions'}
                </span>
                <span
                    className={`text-xs ${cfg.color} opacity-0 group-hover:opacity-100 transition-opacity font-medium`}
                >
                    View →
                </span>
            </div>

...
import { ChevronLeft, Maximize2, ZoomIn, ZoomOut, Zap, Network, PlayCircle, Brain, Share2, Globe, Lock, Copy, RefreshCw, X } from 'lucide-react';
...
export default function KnowledgeMap() {
    ...
    const fetchShareStatus = async () => {
        try {
            const { data: { session } } = await supabase.auth.getSession();
            const res = await fetch('/api/vault/share-map', {
            ...
            console.error('Failed to fetch share status');
        }
    };

    const handleTogglePublic = async (action: 'enable' | 'disable') => {
        setIsSharing(true);
        try {
            const { data: { session } } = await supabase.auth.getSession();
            const res = await fetch('/api/vault/share-map', {
            ...
            if (res.ok) {
                const data = await res.json();
                setShareData({ is_map_public: data.is_map_public, share_token: data.share_token });
                toast.success(action === 'enable' ? 'Knowledge map is now public!' : 'Knowledge map is now private.');
            }
        } catch (e) {
            ...
            <div className="flex flex-col h-[calc(100vh-64px)] bg-[var(--bg)] relative overflow-hidden font-sans">
                ...
                <div className="absolute top-6 right-6 z-10 flex flex-col gap-3">
                    ...
                    <button 
                        ...
                        className="bg-[var(--surface)]/70 backdrop-blur-md border border-[var(--border)] rounded-2xl p-3 flex items-center justify-center text-[var(--text)] hover:bg-[var(--accent)] hover:text-white transition-all shadow-xl group"
                        title="Share Mastery Map"
                    >
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/40 backdrop-blur-sm animate-fade-in">
                        <div className="bg-[var(--surface)] border border-[var(--border)] rounded-3xl shadow-2xl max-w-md w-full p-8 animate-scale-in">
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="text-xl font-display font-medium text-[var(--text)]">Share Mastery Map</h3>
                            ...
                            <div className="space-y-6">
                                <div className="p-4 rounded-2xl bg-[var(--bg)] border border-[var(--border)] flex items-center gap-4">
                                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${shareData.is_map_public ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-400'}`}>
                                        {shareData.is_map_public ? <Globe size={24} /> : <Lock size={24} />}
                                    </div>
                                    <div className="flex-1">
                                        <div className="text-xs font-bold uppercase tracking-widest text-[var(--muted)] mb-0.5">Visibility</div>
                                        <div className="text-sm font-semibold text-[var(--text)]">
                                            {shareData.is_map_public ? 'Public Link Enabled' : 'Private (Only You)'}
                                        </div>
                                    </div>
                                    <button
                                        onClick={() => handleTogglePublic(shareData.is_map_public ? 'disable' : 'enable')}
                                        disabled={isSharing}
                                        className={`px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider transition-all ${
                                            shareData.is_map_public 
                                            ? 'bg-red-50 text-red-600 hover:bg-red-100' 
                                            : 'bg-[var(--accent)] text-white hover:bg-[var(--accent)]/90'
                                        }`}
                                    >
                                        {isSharing ? 'Updating...' : shareData.is_map_public ? 'Disable' : 'Enable'}
                                    </button>
                                </div>
                                ...
                                {shareData.is_map_public && (
                                    <div className="space-y-4 animate-fade-in-up">
                                        <p className="text-xs text-[var(--muted)] leading-relaxed">
                                            Anyone with this link can view a read-only version of your current Knowledge Map and conceptual mastery levels.
                                        </p>
                                        <div className="relative group">
                                            <input 
                                                readOnly 
                                                value={`${window.location.origin}/share/map/${shareData.share_token}`}
                                                className="w-full bg-[var(--bg)] border border-[var(--border)] rounded-xl py-3 pl-4 pr-12 text-xs font-mono text-[var(--text)] focus:outline-none"
                                            />
                                            <button 
                                                onClick={() => {
                                                    navigator.clipboard.writeText(`${window.location.origin}/share/map/${shareData.share_token}`);
                                                    toast.success("Link copied to clipboard!");
                                                }}
                                                className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--accent)] hover:scale-110 transition-transform"
                                                title="Copy Link"
                                            >
                                                <Copy size={16} />
                                            </button>
                                        ...
                                        <div className="flex justify-center">
                                            <button 
                                                onClick={() => handleTogglePublic('enable')}
                                                className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-widest flex items-center gap-1.5 hover:text-[var(--accent)] transition-colors"
                                            >

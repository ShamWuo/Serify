...
function SessionRow({
    ...
    const st = normStatus(session.status);
    const isStale =
        (st === 'processing' || st === 'analyzing') &&
    ...
    function handleRowClick() {
        ...
        if (session.type === 'flow') {
            ...
            router.push(
                st === 'complete' || st === 'completed' || st === 'feedback'
                                ...
                                <table className="w-full text-left border-collapse">
                                    ...
                                    <tbody className="stagger-children">
                                        {filtered.map((session) => {
                                            ...
                                            const rowSt = normStatus(session.status);
                                            return (
                                                <tr
                                                    key={session.id}
                                                    ...
                                                    onClick={() => {
                                                        const isStale =
                                                            (rowSt === 'processing' || rowSt === 'analyzing') &&
                                                            new Date().getTime() - new Date(session.createdAt).getTime() > 60 * 60 * 1000;
                                                        if (isStale) return;
                                                        ...
                                                        if (session.type === 'flow') {
                                                            ...
                                                            router.push(
                                                                rowSt === 'complete' || rowSt === 'completed' || rowSt === 'feedback'
                                                                    ? `/session/${session.id}/feedback`
                                                                    : `/session/${session.id}`
                                                            );
                                                        }
                                                ...
                                                >
                                                    ...
                                                    <td className="py-4 px-2">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div className="min-w-0">
                                                                <div className="font-semibold text-[var(--text)] group-hover:text-[var(--accent)] transition-colors line-clamp-1 text-sm max-w-[200px] md:max-w-[300px]" title={(session?.title || '').replace(/^txt:\/\//, '')}>

...
type Transaction = {
    ...
    amount: number;
    pool: string;
    transaction_type: 'grant' | 'action_debit' | 'expiry' | 'trial_grant';
    action: string;
        ...
        <DashboardLayout>
            ...
            <div className="max-w-4xl mx-auto w-full px-6 md:px-10 py-8 space-y-10 pb-24">
                <header>
                    <Link
                        href="/settings"
                        className="inline-flex items-center text-sm font-medium text-[var(--muted)] hover:text-[var(--text)] mb-6 transition-colors"
                    >
                        <ArrowLeft size={16} className="mr-1" />
                        Back to Settings
                    </Link>
                    <h1 className="text-3xl font-display text-[var(--text)] mb-2 flex items-center gap-3">
                        <Zap className="text-amber-500" size={28} fill="currentColor" /> Spark
                        History
                    </h1>
                    <p className="text-[var(--muted)]">
                        View your recent Spark transactions and usage.
                    </p>
                </header>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6 flex items-center justify-between">
                        <div>
                            <h3 className="text-sm font-medium text-[var(--muted)] uppercase tracking-wider mb-1">
                                Current Balance
                            </h3>
                        ...
                        {loading ? (
                            ...
                            <div className="p-12 text-center text-[var(--muted)] flex items-center justify-center flex-col">
                                ...
                                <p className="text-sm mt-2">
                                    Start a learning session to use Sparks.
                                </p>
                            </div>
                        ) : (
                            transactions.map((tx) => {
                                ...
                                return (
                                    <div
                                        key={tx.id}
                                        className="p-4 sm:p-6 flex items-center justify-between hover:bg-black/5 transition-colors"
                                    >
                                        <div className="flex items-start gap-4">
                                            <div
                                                className={`mt-1 shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${isPositive ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-500'}`}
                                            >
                                                {isPositive ? (
                                            ...
                                            <div>
                                                <p className="font-medium text-[var(--text)] text-sm sm:text-base">
                                                    {formatLabel(
                                                        tx.action || tx.transaction_type
                                                    )}
                                                </p>
                                        ...
                                        <div className="text-right">
                                            <div
                                                className={`font-display font-bold text-lg whitespace-nowrap ${tx.amount > 0 ? 'text-green-600' : 'text-[var(--text)]'}`}
                                            >
                                                {tx.amount > 0 ? '+' : ''}
                                                {tx.amount}
                                            </div>

...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!hasSparks) return res.status(403).json({ error: 'out_of_sparks', message: `You need ${sparkCost} Spark to orchestrate this concept.` });
...
`;
    ...
    await supabaseAdmin.from('flow_sessions').update({ total_sparks_spent: sessionData.total_sparks_spent + sparkCost }).eq('id', sessionId);

    return res.status(200).json({ orchestratorPlan, total_sparks_spent: sessionData.total_sparks_spent + sparkCost });

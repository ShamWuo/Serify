...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        if (sessionError || !sessionData) return res.status(404).json({ error: 'Session not found' });
        ...
        if (!progressData || !progressData.orchestrator_plan) {
            return res.status(400).json({ error: 'Orchestrator plan not initialized. Call /api/flow/orchestrate first.' });
        }
        ...
        const lastStep = (previousSteps && previousSteps.length > 0) ? previousSteps[previousSteps.length - 1] : null;
        ...
        if (!lastStep) {
            ...
            if (lastStep.response_type === 'needs_work' && plan.anchor.alternativeText && !lastStep.content.isAlternative) {
                nextStepType = 'anchor';
                content = { text: plan.anchor.alternativeText, form: plan.anchor.form, isAlternative: true };
            } else {
                nextStepType = 'check';
                content = plan.checks?.[0] || { questionText: "How would you summarize this?", checkType: "recall" };
            }
        } else if (lastStep.step_type === 'check') {
            if (!lastStep.evaluation) return res.status(400).json({ error: 'Check step not evaluated yet' });

            if (['A', 'strong'].includes(lastStep.evaluation.path) || (lastStep.evaluation.outcome === 'strong')) {

                const currentCheckIndex = plan.checks?.findIndex((c: any) => c.questionText === lastStep.content.questionText) ?? 0;
                ...
                if (lastStep.evaluation.nextReinforceContent) {
                    ...
                    content = { text: lastStep.evaluation.nextReinforceContent, path: lastStep.evaluation.path };
                } else {
            ...
            if (!lastStep.evaluation) return res.status(400).json({ error: 'Confirm step not evaluated yet' });

            if (['A', 'strong'].includes(lastStep.evaluation.path) || (lastStep.evaluation.outcome === 'strong') || lastStep.evaluation.masterySignal === 'solid' || lastStep.evaluation.masterySignal === 'developing') {

                await supabaseAdmin.from('flow_concept_progress').update({ status: 'completed' }).eq('id', progressData.id);
                nextStepType = 'completed';
                content = { text: "Concept sequence completed!" };
            } else {
                const confirmAttempts = previousSteps?.filter(s => s.step_type === 'confirm').length || 1;
            ...
            return res.status(200).json({ action: 'concept_complete', totalSparksSpent: sessionData.total_sparks_spent });
        } else {

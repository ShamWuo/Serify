...
export default async function handler(
    ...
    try {
        const [
            ...
            supabase.from('deep_dive_lessons').select('*').eq('session_id', sessionId).maybeSingle(),
            supabase.from('concept_explanations').select('concept_id').eq('session_id', sessionId),
            supabase.from('tutor_conversations').select('*').eq('session_id', sessionId).maybeSingle(),
            supabase.from('feynman_attempts').select('feedback').eq('session_id', sessionId).order('attempt_number', { ascending: true })
        ]);
        ...
        const materials = {
            ...
            feynman: {
                ...
                latestAssessment: feynmanAttempts && feynmanAttempts.length > 0

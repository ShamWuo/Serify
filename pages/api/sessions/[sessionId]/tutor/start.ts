...
export default async function handler(
    ...
    if (!UUID_REGEX.test(sessionId)) {
        return res.status(400).json({ error: 'Session ID is not a valid UUID. This session predates AI Tutor support.' });
    }
    ...
    try {
        ...
        if (!hasSparks) {
            return res.status(403).json({ error: 'out_of_sparks', message: `You need ${sparkCost} Spark to start an AI Tutor session.` });
        }

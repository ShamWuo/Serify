...
export default function FlashcardsMode() {
    ...
    useEffect(() => {
        ...
        const initDeck = async () => {
            try {
                ...
                const weakConcepts = (parsed.report?.strength_map || []).filter(
    ...
    if (!loading && cards.length === 0 && !isComplete) {
        return (
            <div className="min-h-screen bg-[var(--background)] text-[var(--text)] flex flex-col pt-12">
                <div className="max-w-[600px] mx-auto w-full px-6 flex-1 flex flex-col items-center pt-24 text-center">
                    <div className="w-20 h-20 rounded-full bg-amber-50 text-amber-500 flex items-center justify-center mb-6 text-3xl">🎴</div>
                    <h2 className="text-3xl font-display mb-4">No cards to review</h2>
                    <p className="text-[var(--muted)] mb-8 text-lg">
                        We couldn&apos;t generate cards for this session. It might be because you already understood everything or the content was too short.
                    </p>
        ...
        return (
            <div className="min-h-screen bg-[var(--background)] text-[var(--text)] flex flex-col pt-12">
                <div className="max-w-[600px] mx-auto w-full px-6 flex-1 flex flex-col items-center pt-24">
                    <div className="w-20 h-20 rounded-full bg-[var(--accent-light)] text-[var(--accent)] flex items-center justify-center mb-6 text-3xl">🎉</div>
                    <h2 className="text-3xl font-display mb-4">Deck Complete</h2>
                    <p className="text-[var(--muted)] text-center mb-8 text-lg">
                        You retired {stats.gotIt} cards. {stats.shaky > 0 && `You have ${stats.shaky} shaky cards to review later.`}
                    </p>
                    <Link href={`/session/${id}/feedback`} className="px-6 py-3 bg-[var(--accent)] text-white rounded-xl font-medium hover:-translate-y-0.5 transition-all">
                        Return to Report &rarr;
                    </Link>
        ...
        <div className="min-h-screen bg-[var(--background)] text-[var(--text)] flex flex-col">
            ...
            <header className="px-6 py-5 border-b border-[var(--border)] flex items-center justify-between bg-white/50 backdrop-blur-sm sticky top-0 z-10 hidden md:flex">
                ...
                <div className="font-medium text-sm text-[var(--text)]">
                    <span className="font-bold">{getConceptName(currentCard.conceptId)}</span> <span className="text-[var(--muted)] mx-2">|</span> Card {currentIndex + 1} of {cards.length}
                </div>
            ...
            <main className="flex-1 flex flex-col items-center justify-center p-6 pb-32">
                ...
                <div className="md:hidden text-center mb-8">
                    ...
                    <div className="text-[var(--muted)] text-sm">Card {currentIndex + 1} of {cards.length}</div>
                </div>
                ...
                >
                    <div className={`absolute inset-0 w-full h-full transition-transform duration-500 [transform-style:preserve-3d] ${isFlipped ? '[transform:rotateX(180deg)]' : ''}`}>
                        ...
                        <div className="absolute inset-0 w-full h-full backface-hidden p-8 md:p-12 flex flex-col items-center justify-center text-center bg-white rounded-3xl">
                            <h2 className="text-2xl md:text-[32px] font-display mb-6 text-[var(--text)] leading-tight">{currentCard.front}</h2>
                            <p className="text-[var(--muted)] text-sm absolute bottom-8 opacity-60 font-medium tracking-wide">TAP TO FLIP</p>
                        </div>
                        ...
                        <div className="absolute inset-0 w-full h-full backface-hidden p-8 md:p-12 flex flex-col items-center justify-center text-center bg-white rounded-3xl pt-16 [transform:rotateX(180deg)] shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)] border-2 border-[var(--accent)]">
                            <div className="absolute top-6 left-0 right-0 flex justify-center">
                                <span className="text-xs font-bold uppercase tracking-widest text-[var(--accent)] bg-[var(--accent-light)] px-3 py-1 rounded-full">Answer</span>
                            </div>
                            <p className="text-lg md:text-xl text-[var(--text)] leading-relaxed">{currentCard.back}</p>
                        </div>

        ...
        const initDeck = async () => {
            try {
                ...
                const wk = (parsed.report?.strength_map || [])
                    .filter((item: any) =>
                        ['revisit', 'shaky', 'skipped'].includes(item.mastery_state)
                    )
                    .map((item: any) => ({
                        id: item.concept_id,
                        name:
                ...
                if (proStatus) {
                    ...
                    const startRes = await fetch(
                        `/api/sessions/${id}/tutor/start${isRegenerating ? '?regenerate=true' : ''}`,
                        {
                            method: 'POST',
                            headers
                        }
                        ...
                        if (data.messages && data.messages.length > 0) {
                            ...
                            if (wk.length > 0) {
                                setMessages([
                                    {
                                        role: 'model',
                                        content: `Hi there. I noticed you had some trouble with **${wk[0].name}** during your last session. Specifically, it looked like: *${wk[0].feedbackNote}*\n\nHow can I help clear that up for you? We can walk through it step-by-step or do a quick practice.`
                                    }
                                ]);
                            } else {
                                setMessages([
                                    {
                                        role: 'model',
                                        content:
                                            'Hi! You did great in your last session. What would you like to drill into today?'
                                    }
                                ]);
                            }
                        }
    ...
    const sendMessage = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setMessages((prev) => [
                    ...prev,
                    { role: 'model', content: 'Sorry, I ran into an error connecting to my brain.' }
                ]);
            }
        } catch (e) {
            console.error(e);
            setMessages((prev) => [
                ...prev,
                { role: 'model', content: 'Sorry, I ran into a network error.' }
            ]);
        } finally {
            setSending(false);
        }
        ...
        return (
            <div className="min-h-screen bg-[var(--background)] text-[var(--text)] flex flex-col pt-12">
                <div className="max-w-[500px] mx-auto w-full px-6 flex-1 flex flex-col items-center pt-24 text-center">
                    ...
                    <Link
                        href={`/session/${id}/feedback`}
                        className="px-8 py-3.5 bg-[var(--surface)] border border-[var(--border)] text-[var(--text)] rounded-xl font-medium hover:bg-black/5 transition-colors"
                    >
                        Return to Report
                    </Link>
                </div>

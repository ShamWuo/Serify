...
import {
    Search, Filter, Clock, Youtube, FileUp, FileText, ClipboardPaste,
    Layers, Target, BookOpen, MessageSquare, Bot, Edit3, Zap, Trash2,
    X, AlertTriangle, ChevronRight, FlaskConical, CheckCircle2
} from 'lucide-react';
...
function getIcon(contentType: string) {
    switch (contentType) {
        ...
        case 'pdf': return <FileUp className="text-blue-500" size={18} />;
        case 'article': return <FileText className="text-green-500" size={18} />;
        case 'notes': return <ClipboardPaste className="text-orange-500" size={18} />;
        case 'flow': return <Zap className="text-purple-500" size={18} />;
...
function DeleteModal({ session, onConfirm, onCancel, deleting }: {
            ...
            <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-2xl max-w-md w-full p-6">
                <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                        <h2 className="text-lg font-bold text-[var(--text)] mb-1">Delete Session</h2>
                        <p className="text-sm text-[var(--muted)] leading-relaxed">
                            <span className="font-semibold text-[var(--text)]">"{session.title}"</span> and all its associated
                            {session.type === 'reflection' ? ' flashcards, quizzes, explanations, and tutor conversations' : ' steps and progress'}
...
function SessionRow({ session, onDelete }: { session: UnifiedSession; onDelete: (s: UnifiedSession) => void }) {
    ...
    function handleRowClick() {
        if (session.type === 'flow') {
            ...
            router.push(session.status === 'complete' || session.status === 'completed'
                ? `/session/${session.id}/feedback`
                : `/session/${session.id}`);
        }
    }

    const statusLabel = session.type === 'flow'
        ? `${session.completedCount ?? 0}/${session.totalCount ?? 0} concepts`
        : session.status === 'complete' || session.status === 'completed' ? 'Completed' : 'In Progress';

    const statusColor = session.type === 'flow'
    ...
    return (
        <tr className="border-b border-[var(--border)] hover:bg-black/[0.025] transition-colors group">
            ...
            <td className="py-4 px-6 cursor-pointer" onClick={handleRowClick}>
                <div className="flex items-center gap-3">
                    ...
                    {session.type === 'reflection' && (
                        <div className="flex items-center gap-1.5 ml-2 hidden lg:flex" onClick={e => e.stopPropagation()}>
                            {[
                                { label: 'Flashcards', icon: <Layers size={10} />, href: `/learn/${session.id}/flashcards`, cls: 'bg-blue-50 text-blue-600 border-blue-200 hover:bg-blue-100' },
                                ...
                                { label: 'Explain', icon: <MessageSquare size={10} />, href: `/learn/${session.id}/explain`, cls: 'bg-purple-50 text-purple-600 border-purple-200 hover:bg-purple-100' },
                                ...
                                { label: 'Feynman', icon: <Edit3 size={10} />, href: `/learn/${session.id}/feynman`, cls: 'bg-rose-50 text-rose-600 border-rose-200 hover:bg-rose-100' },
                            ].map(m => (
                                <Link
                                    key={m.label}
                                    href={m.href}
                    ...
                    {session.type === 'flow' && (
                        <div className="ml-2 hidden lg:flex items-center" onClick={e => e.stopPropagation()}>
                            <Link
            ...
            {}
            <td className="py-4 px-4 text-sm text-[var(--muted)] whitespace-nowrap hidden sm:table-cell cursor-pointer" onClick={handleRowClick}>
                {formatDate(session.createdAt)}
            </td>
            {}
            <td className="py-4 px-4 cursor-pointer" onClick={handleRowClick}>
                <span className={`inline-flex items-center text-xs font-bold px-2 py-1 rounded-lg border ${statusColor}`}>
                    {statusLabel}
                </span>
            </td>
            {}
...
export default function LibraryPage() {
    ...
    const filtered = sessions.filter(s => {
        ...
        if (filterTab === 'completed') {
            matchesTab = s.status === 'complete' || s.status === 'completed' || (s.type === 'flow' && s.completedCount === s.totalCount);
        } else if (filterTab === 'in_progress') {
            matchesTab = s.status !== 'complete' && s.status !== 'completed' && !(s.type === 'flow' && s.completedCount === s.totalCount);
        }
        const matchesSearch = !search.trim() || s.title.toLowerCase().includes(search.toLowerCase());
    ...
    const completedCount = sessions.filter(s => s.status === 'complete' || s.status === 'completed' || (s.type === 'flow' && s.completedCount === s.totalCount)).length;
                ...
                <div className="flex flex-wrap items-center gap-1 bg-[var(--surface)] border border-[var(--border)] rounded-xl p-1 mb-6 w-fit">
                    {([
                        ...
                        >
                            ...
                            {tab === 'completed' && <CheckCircle2 size={12} className={filterTab === tab ? '' : 'text-emerald-500'} />}
                            {label}
                            <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${filterTab === tab ? 'bg-white/20 text-white' : 'bg-[var(--border)] text-[var(--muted)]'}`}>
                                {count}
                            </span>
                        </button>
                    ))}
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-[var(--border)] bg-black/[0.02]">
                                    <th className="py-3 px-6 font-medium text-xs uppercase text-[var(--muted)] tracking-wider">Session</th>
                                    <th className="py-3 px-4 font-medium text-xs uppercase text-[var(--muted)] tracking-wider hidden sm:table-cell">Date</th>
                                    <th className="py-3 px-4 font-medium text-xs uppercase text-[var(--muted)] tracking-wider">Status</th>
                                    <th className="py-3 px-4 text-right" />
                                </tr>
                            </thead>
                            <tbody>
                                {filtered.map(session => (
                                    <SessionRow key={session.id} session={session} onDelete={setDeleteTarget} />
                                ))}
                            </tbody>
                        </table>
                    ) : (
                        <div className="flex flex-col items-center justify-center py-20 text-center px-6">
                            <div className="w-16 h-16 bg-[var(--border)] rounded-xl flex items-center justify-center mb-4">
                                <Search size={22} className="text-[var(--muted)]" />
                            </div>
                            <h3 className="text-lg font-bold text-[var(--text)] mb-1">
                                {search ? 'No sessions found' :
                                    filterTab === 'reflection' ? 'No Analysis Sessions' :
                                        filterTab === 'flow' ? 'No Flow Sessions' :
                                            filterTab === 'completed' ? 'Nothing finished yet' :
                                                filterTab === 'in_progress' ? 'All caught up' :
                            ...
                            {!search && (
                                <div className="flex gap-3 mt-6">
                                    <Link href="/analyze" className="px-5 py-2.5 bg-[var(--accent)] text-white rounded-xl text-sm font-medium hover:bg-[var(--accent)]/90 transition-colors shadow-md shadow-[var(--accent)]/20">
                                        Analyze Content
                                    </Link>
                                    <Link href="/flow" className="px-5 py-2.5 bg-[var(--surface)] border border-[var(--border)] text-[var(--text)] rounded-xl text-sm font-medium hover:border-purple-300 hover:text-purple-600 transition-colors flex items-center gap-1.5">

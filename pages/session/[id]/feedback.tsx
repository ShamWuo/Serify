    ...
    useEffect(() => {
        ...
        async function loadReport() {
            try {
                ...
                if (existingAnalysis) {
                    const insights = existingAnalysis.insights as any;
                    setReport({
                        summary_sentence: insights?.summary,
                        strength_map: existingAnalysis.strength_map,
                        cognitive_analysis: insights?.cognitive_analysis,
                        overall_counts: insights?.overall_counts,
                        misconception_displayReport: insights?.misconception_report,
                        focus_suggestions: existingAnalysis.focus_suggestions?.map((s: string) => {
                            const [title, ...reasonParts] = s.split(': ');
                            return { title, reason: reasonParts.join(': ') };
                        })
                    });
                    return;
                }

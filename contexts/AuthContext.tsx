...
async function loadProfile(userId: string, email: string): Promise<UserProfile | null> {
  ...
  if (!profile) {
    console.warn(`Creating fallback profile for ${userId} because profiles table returned null`);
    const fallbackName = email ? email.split('@')[0].charAt(0).toUpperCase() + email.split('@')[0].slice(1) : 'User';
  ...
  const formatDisplayName = (name: string | null | undefined, email: string | undefined): string => {
    if (name && name.trim()) {
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (isOAuth) {
        console.log('AuthContext: Detected OAuth callback. Waiting for AuthStateChange to fire...');
  ...
  return (
    <AuthContext.Provider value={{ user, loading, login, register, loginWithGoogle, logout, updatePreferences, markOnboardingComplete }}>
      {children}
